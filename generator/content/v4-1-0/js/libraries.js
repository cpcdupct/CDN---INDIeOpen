...
(function () {
	...
	fetch(`https://cdn-cpcd-upct-e2bdhub5efg7c9hk.z01.azurefd.net/$web/generator/content/v4-1-0/languages/${currLang}.json`)
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        function newNote() {
            ...
            $('.note>.note_cnt>textarea.title').change(function () {
            ...
            $('.note>.note_cnt>textarea.description').change(function () {
                var note_edited = $(this).parent().parent().attr('data-time');
                if (notes_global.length > 0) {
                    for (var i = 0; i < notes_global.length; i++) {
...
(function () {
	...
	let onLanguageLoaded = function (err, i18n) {
		...
		let processVideo = function () {
			...
			player.player.addEventListener(amp.eventName.loadstart, function () { $(self).find('.vjs-text-track-display').attr('aria-live', 'off'); });
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        let init = function () {
            ...
            $widget.closest('.content-element').on('click', '.btn-check', function () {
                ...
                let gamification = $('body').hasClass('gamification');
                let $terms = $(this).closest('.content-element').find('.match-column .term-button');
                let total = $terms.length;
                let $columns = $(this).closest('.content-element').find('.match-column .column-list');
                let score = $terms.filter(function () {
                    let idx = $columns.index($(this).closest('.column-list'));
                    return solutions[idx].includes($(this).data('term'));
                }).length
                $(this).trigger('gamificate', [score === total, score / total]);
                if (!gamification) {
        ...
        $('.widget-puzzle').each(function () {
            ...
            tmpImage.onload = function () {
                ...
                $widget.closest('.content-element').on('click', '.btn-check', function () {
                    ...
                    let gamification = $('body').hasClass('gamification');
                    let score = $(this).closest('.content-element').find('.piece-wrapper .piece').filter(function (idx) {
